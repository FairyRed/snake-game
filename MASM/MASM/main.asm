...
.data
	map WORD 1600 DUP (0) ;地圖大小(20*80)
	row byte 1600 DUP (0) ;蛇的row(20*80)
	col byte 1600 DUP (0) ;蛇的col(20*80)
...
ProduceFood PROC USES EAX EBX EDX
	...
	mov eax,19	;產生0~18任意數字，因為18為牆壁，故省略
	call RandomRange
	mov dh,al
...
InitialSnake PROC Uses esi EDX EBX	;此程序為預先建立蛇的身體
	mov esi ,1			;將1存到esi
	mov row[esi],16		;將row的第一位設為頭的位置
	mov col[esi],50		;將col的第一位設為頭的位置
	mov dh,16			;將16存入DH
	mov dl,50			;將50存入DL
	mov bx,1			;我們設定1為蛇的身體，放入BX
	call storevalue		;將16、50的該位置的身體值存入map中
	mov esi,2			;將1存到esi
	mov row[esi],17		;將row的第二位設為蛇尾的位置
	mov col[esi],50		;將col的第二位設為蛇尾的位置
	mov dh,17			;將17存入DH
	mov dl,50			;將50存入DL
	mov bx,1			;我們設定1為蛇的身體，放入BX
	call storevalue		;將17、50的該位置的身體值存入map中
	ret					;返回
InitialSnake ENDP
;-----------------------------------------------------------------
...
CreateMap PROC	;建立牆壁
	...
	mov dh,0			;將0存入DH
	mov ecx,20   		;將24存入ECX，此迴圈為建立上下邊界之牆壁
	column_wall:
		mov dl,0		;左邊界牆壁
		call StoreValue	;儲存該位置為牆壁
		mov dl,79		;右邊界牆壁
